...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === 'facture') {
        ...
        try { await interaction.deferReply({ flags: 64 }); } catch(e) { console.error('❌ deferReply échoué:', e.message); return; }
        ...
        try {
            ...
            const dupRes = await sheets.spreadsheets.batchUpdate({
                ...
                resource: { requests: [{ duplicateSheet: {
                    ...
                    newSheetName: 'FACT-' + agent[1].substring(0,15).replace(/ /g,'-') + '-' + new Date().toLocaleDateString('fr-FR', {month:'2-digit', year:'numeric'}).replace('/',''),
                }}]}
            });
