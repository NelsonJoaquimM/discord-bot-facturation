...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(opt  => opt.setName('tarif').setDescription('Tarif ligne 1')
      .addChoices({name:'18€',value:'18'},{name:'17.50€',value:'17.5'}).setRequired(true))
    .addIntegerOption(opt => opt.setName('qte50plus').setDescription('Nombre de RDV (si +50 RDV)').setRequired(true))
    .addStringOption(opt  => opt.setName('tarifplus').setDescription('Tarif ligne 2')
      .addChoices({name:'23€',value:'23'},{name:'22.50€',value:'22.5'}).setRequired(true))
    .addStringOption(opt  => opt.setName('date').setDescription('Date (ex: 08/03/2024)').setRequired(false))
    .addStringOption(opt  => opt.setName('ligne').setDescription('Déduire 49€ ligne ON/OFF ?')
      .addChoices({name:'Oui',value:'oui'},{name:'Non',value:'non'}).setRequired(false)),
...
async function envoyerNotifFactures(resultats, nomDossier) {
  ...
  const message =
    `## 🧾 Génération automatique des factures — ${nomDossier}\n` +
...
client.once('ready', async () => {
  ...
  setInterval(async () => {
    const now = new Date();
    // Le 8 du mois à 08h00 (heure Paris = UTC+2)
    const heureLocale = now.getUTCHours() + 2;
    if (now.getUTCDate() === 8 && heureLocale === 8 && now.getUTCMinutes() === 0) {
      ...
      try {
        ...
        console.error('Erreur auto factures:', err.message);
      }
    }
  }, 60 * 1000);

  console.log('📅 Vérification automatique activée — génération le 8 de chaque mois à 08h00');
});
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isModalSubmit() && interaction.customId.startsWith('modal_banque|')) {
    ...
    try {
      ...
      const res      = await sheets.spreadsheets.values.get({ spreadsheetId: process.env.SPREADSHEET_ID, range: 'PROFILS!A2:L' });
      const rows     = res.data.values || [];
      ...
      if (rowIndex === -1) {
        await sheets.spreadsheets.values.append({
          spreadsheetId: process.env.SPREADSHEET_ID, range: 'PROFILS!A2',
          valueInputOption: 'USER_ENTERED', resource: { values: [newRow] },
        });
      } else {
        await sheets.spreadsheets.values.update({
          spreadsheetId: process.env.SPREADSHEET_ID, range: `PROFILS!A${rowIndex + 2}`,
          valueInputOption: 'USER_ENTERED', resource: { values: [newRow] },
        });
      }
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'generer-factures') {
    await interaction.deferReply({ ephemeral: true });
    await interaction.editReply('⏳ Génération en cours... Tu recevras la notification dans le salon dédié.');
